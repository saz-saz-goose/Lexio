    ...
    await sbClient.from('profiles').update({
        ...
        function conjugateVerb(infinitive, tense, isIrregular) {
            ...
            const conj = {
                ...
                present_perfect: () => {
                    let pp;
                    if (isIrregular && IRREGULAR_PP[v]) pp = IRREGULAR_PP[v];
        ...
        // All irregular verbs across all levels — used for correct conjugation regardless of selected level
        const ALL_IRREGULAR_VERBS = new Set([
            ...Object.values(VERB_LISTS.irregular).flat()
        ]);
        ...
        function generateTenseChoiceSentences(level) {
            ...
            for (const tenseKey of tensePool) {
                ...
                const bank = TENSE_SENTENCE_BANK[tenseKey];
                if (!bank || bank.length === 0) continue;

                const template = bank[Math.floor(Math.random() * bank.length)];
                const verb = template.verb;
                const isIrr = ALL_IRREGULAR_VERBS.has(verb); // always check global set, not level list
                ...
                for (const dt of shuffle(tenses.filter(t => t !== tenseKey))) {
                    if (TENSE_SENTENCE_BANK[dt]?.[0]?.goingTo) continue;
        ...
        function getVerbListForTense(tenseKey) {
            ...
            const reg = VERB_LISTS.regular[level] || VERB_LISTS.regular.A1;
            const irr = VERB_LISTS.irregular[level] || VERB_LISTS.irregular.A1;
        ...
        function loadDrillItem() {
            ...
            const isIrr = ALL_IRREGULAR_VERBS.has(verb);
