    ...
    document.getElementById('match-counter').textContent = `${matched} / ${words.length} paires trouvées`;
        ...
        async function initializeDashboard() {
            if (currentProfile) {
                ...
                document.getElementById('header-avatar-btn').innerText = getAvatarEmoji(currentProfile.avatar_id);
            } else {
            ...
            try {
                ...
                if (currentProfile) {
                    await fetchLeaderboard(WORD_LIST_FILE || null);
                }
...
async function fetchLeaderboard(courseKey) {
    ...
    const { data, error } = await sbClient
        .from('profiles')
        .select('username, avatar_id, courses_progress, total_score, id');
    ...
    if (courseKey) {
        ...
        ranked = data
            .filter(p => p.courses_progress?.[courseKey]?.score > 0)
            .map(p => ({
                ...p,
                displayScore: p.courses_progress[courseKey].score
            }))
            .sort((a, b) => b.displayScore - a.displayScore)
            .slice(0, 10);
    } else {
        ...
        ranked = data
            .map(p => ({ ...p, displayScore: p.total_score || 0 }))
            .sort((a, b) => b.displayScore - a.displayScore)
            .slice(0, 10);
    }
